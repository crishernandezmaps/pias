treeJSON = d3.json("dendo.json", function(error, treeData) {
  var totalNodes = 0;
  ...
  visit(treeData, function(d) {
      totalNodes++;
      maxLabelLength = Math.max(d.name.length, maxLabelLength) - 0.5;
    },
    function(d) {
  ...
  function update(source) {
    ...
    node.select("circle.nodeCircle").attr("r", 7).style("fill", function(d) {
      return d._children ? "lightsteelblue" : "#fff";
    });
